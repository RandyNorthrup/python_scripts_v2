...
```bash
python3 meal_portion.py
python3 meal_portion.py --grams-per-person 225
```

Run `python3 meal_portion.py --help` for every option. Interactive quizzes and games only prompt when `--play` is supplied.
