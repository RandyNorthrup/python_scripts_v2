...
```bash
python3 pet_food_plan.py
python3 pet_food_plan.py --grams-per-day 400
```

Run `python3 pet_food_plan.py --help` for every option. Interactive quizzes and games only prompt when `--play` is supplied.
