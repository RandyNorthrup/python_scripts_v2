...
```bash
python3 filename_sanitizer_preview.py
python3 filename_sanitizer_preview.py /path/to/file-or-directory
```

Run `python3 filename_sanitizer_preview.py --help` for every option. Interactive quizzes and games only prompt when `--play` is supplied.
